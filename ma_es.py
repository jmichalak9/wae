"""This module contains implementation of MA-ES"""
import numpy as np
from cma_es import CMAES


class MAES(CMAES):
  """Implementation of MA-ES"""
  ...
  def __init__(self, N, offspring_size: int, seed: int, display_result=False):
    ...
    self.mu = int(
      np.floor(self.offspring_size / 2.0))  # Number of best individuals
    ...
    self.damping = 1 + self.cs + 2.0 * np.maximum(0.0, np.sqrt(
      (self.mu_eff - 1.0) / (N + 1.0)) - 1.0)
  ...
  def calculate(self, y, sigma, fun, max_iterations: int):
    ...
    while True:
      ...
      s = (1 - self.cs) * s + np.sqrt(
        self.mu_eff * self.cs * (2 - self.cs)) * self.recombination(zz)
      I = np.identity(self.N)
      M = M @ (I + (self.c1 / 2) * (np.outer(s, s.T) - I) + (self.cw / 2) *
               (self.recombination_with_transposition(zz) - I))

      sigma *= np.exp(
        self.cs / self.damping * (np.linalg.norm(s) / self.chi_n - 1))
