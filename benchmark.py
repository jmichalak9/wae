...
See the code:
`<https://github.com/numbbo/coco/blob/master/code-experiments/build/python/example_experiment2.py>`_

See a beginners example experiment:
`<https://github.com/numbbo/coco/blob/master/code-experiments/build/python/example_experiment_for_beginners.py>`_
...
# for more suite filter options see
# http://numbbo.github.io/coco-doc/C/#suite-parameters
suite_year_option = ''  # "year: 2022"  # determine instances by year

# number of batches, batch=3/32 works to set both, current_batch and batches
batches = 1
...
for batch_counter, problem in enumerate(
  ...
  # callable, all zeros in first call
  propose_x0 = problem.initial_solution_proposal
  evalsleft = lambda: int(problem.dimension * budget_multiplier + 1 -
                          max((problem.evaluations,
  ...
  minimal_print(problem, restarted=irestart,
