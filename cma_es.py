"""This module contains implementation of CMA-ES"""
import warnings
import numpy as np
...
class CMAES:
  ...
  def __init__(self, N, seed: int, display_result=False):
    ...
    self.mu = int(np.floor(self.offspring_size / 2.0))
    ...
    # mean of chi distribution with N degrees of freedom
    # using Stirling's approximation
    self.chi_n = np.sqrt(N) * (1.0 - 1.0 / (4.0 * N) + 1.0 / (21.0 * N ** 2))
    self.damping = 1 + self.cs + 2.0 * np.maximum(0.0,
                                                  np.sqrt((self.mu_eff - 1.0) /
                                                          (N + 1.0)) - 1.0)
  ...
  def calculate(self, y, sigma, fun, max_iterations: int):
    ...
    while True:
      ...
      s = (1 - self.cs) * s + np.sqrt(self.mu_eff * self.cs * (2 - self.cs)) * \
          self.recombination(zz)

      hsig = np.linalg.norm(s) / np.sqrt(
        1 - (1 - s) ** (2 * counteval / self.offspring_size)) / self.chi_n \
             < 1.4 + 2 / (self.N + 1)
      p = (1 - self.cp) * p + np.sqrt(self.mu_eff * self.cp * (2 - self.cp)) \
          * self.recombination(dd)

      covariance_matrix = (1 - self.c1 - self.cw) * covariance_matrix + \
                          self.c1 * np.outer(p, p.T) + \
                          (1 - hsig) * self.cp * (2 - self.cp) \
                          * covariance_matrix + self.cw \
                          * self.recombination_with_transposition(dd)

      sigma *= np.exp(self.cs / self.damping * (np.linalg.norm(s)
                                                / self.chi_n - 1))

      if counteval - eigeneval > self.offspring_size / (self.cw + self.c1) \
          / self.N / 10:
        eigeneval = counteval
        covariance_matrix = np.triu(covariance_matrix) \
                            + np.triu(covariance_matrix, 1)
